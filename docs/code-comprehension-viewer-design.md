...
3. **Stable ordering.** Children order by kind group (types → fns → classes → modules), then height descending within group, then name; hierarchical stability holds (a container's layout depends only on its own children). Amended 2026-07-11: size may change slot within a container (packing density won over slot memory — names stay pinned at every zoom); sibling subtrees are still unaffected by each other's internal edits.
